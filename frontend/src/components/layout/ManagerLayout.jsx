...
import { LogOut, LayoutDashboard, Utensils, Grid2X2, Users, FileBarChart, Menu as MenuIcon, Home, ChevronLeft, CalendarClock, Tag } from 'lucide-react';
...
const ManagerLayout = () => {
  ...
  const navItems = [
    ...
    { name: 'Voucher',      path: '/manager/vouchers',     icon: Tag },
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] text-[#0f172a] font-sans selection:bg-primary-500 selection:text-white flex">
      ...
      {/* ── SIDEBAR ───────────────────────────────────────────── */}
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#0f172a] text-white flex flex-col transition-transform duration-300 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:static md:inset-auto`}>

        {/* Logo — click về trang chủ */}
        <div className="flex items-center justify-center h-20 border-b border-[#1e293b] shrink-0 px-4">
        ...
        <div className="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
          ...
          {/* Nút về trang chủ khách */}
          <Link
            to="/"
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-[#64748b] hover:bg-[#1e293b] hover:text-[#94a3b8] transition-all duration-200 mb-3 border border-[#1e293b]"
          >
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        {/* Mobile Header */}
        <div className="md:hidden flex items-center justify-between h-16 px-4 bg-white border-b border-[#e2e8f0] shrink-0">
