...
import { LogOut, LayoutDashboard, Utensils, Grid2X2, Users, FileBarChart, Menu as MenuIcon } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#f1f5f9] text-[#0f172a] font-sans selection:bg-primary-500 selection:text-white flex">
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#0f172a] text-white transition-transform duration-300 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:static md:inset-auto`}>
        <div className="flex items-center justify-center h-20 border-b border-[#1e293b]">
          <span className="font-admin font-bold text-xl tracking-wide text-white">RESTAURANT<span className="text-primary-500">.</span></span>
        </div>
        <div className="px-4 py-6 space-y-2">
          <p className="text-xs font-bold text-[#64748b] uppercase tracking-wider mb-4 px-3">Quản trị viên</p>
          {navItems.map((item) => {
